...
public class StartRangeDateTransformer implements ITransformer {
    ...
    public TypedValue transform(String value, String lang, ValueType type, String unit) throws TransformException {
        try {
            ...
            String stringDate="";
            if(GenericTransformer.detectDateType(strs[0]).equals("shamsi"))
                stringDate=GenericTransformer.shamsiTransformer(strs[0]);
            else if(GenericTransformer.detectDateType(strs[0]).equals("miladi"))
                stringDate=GenericTransformer.miladiTransformer(strs[0]);
